...
import { getUser } from "@/hooks/users/use-get-user.hook";
...
export const Perfil = () => {
  ...
  function mascaraEmail(email: string) {
    const [nome, dominio] = email.split("@");
    const nomeMascarado = nome.slice(0, 2) + "****";
  ...
  return (
    <div className=" flex flex-col justify-between bg-gradient-to-r from-blue-600/10 via-purple-600/10 to-pink-600/10 min-h-screen">
      <NavBar />
      {/* Cabeçalho */}
      <div className="p-6 container mx-auto">
        <div className="bg-white p-6 rounded-lg shadow mb-6 flex items-center gap-4">
          <div className="bg-gray-300 w-16 h-16 rounded-full flex items-center justify-center text-xl font-bold">
            {obterIniciais(data?.name ?? "Usuário")}
          </div>
          <div>
            <h1 className="text-lg font-semibold">{data?.name}</h1>
            <p className="text-gray-500">
              {mascaraEmail(data?.email ?? "usuario****@gmail.com")}
            </p>
          </div>
        </div>
